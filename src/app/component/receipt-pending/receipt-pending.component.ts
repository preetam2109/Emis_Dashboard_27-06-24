...
@Component({
  ...
  templateUrl: './receipt-pending.component.html',
  styleUrls: ['./receipt-pending.component.css']
})
export class ReceiptPendingComponent {
...
constructor(private spinner: NgxSpinnerService,private api: ApiService, private http: HttpClient,private breakpointObserver:BreakpointObserver,private cdr:ChangeDetectorRef) {
  this.chartOptions ={
   ...
   title: {
     text: 'Online Receipt Pending Summary Report',
     align:'center'
   },
   tooltip: {
     y: {
...
loadData(): void {
  this.spinner.show();
  this.api.receiptPendingSummary().subscribe(
    (data: any) => {
